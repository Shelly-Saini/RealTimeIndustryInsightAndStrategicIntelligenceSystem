...
def calculate_trend_frequency(trend_results, brands):
    ...
    for brand in brands:
        ...
        for i, item in enumerate(trend_results):
            title = item.get("title", "").lower()
            snippet = item.get("snippet", "").lower()

            position_weight = max(1, 10 - i)  # higher rank = higher weight

            if brand.lower() in title:
                score += 3 * position_weight
            if brand.lower() in snippet:
    ...
    with tab2:
        ...
        if trend_freq:
            ...
            fig_bar = go.Figure(go.Bar(
                ...
                marker=dict(
                    color=[BRAND_COLORS[i % len(BRAND_COLORS)] for i in range(len(chart_brands))],
                    line=dict(width=0),
                ),
